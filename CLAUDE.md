...
**规则**：执行者在交付报告中声明"文档已更新"时，必须附上以下三类证据之一，否则视为未完成：

1. **diff 摘要**：`git diff --stat` 的输出摘要（必须包含改动文件清单及增删行数统计）
2. **新增章节完整内容**：如果是新增章节，必须贴出完整内容
3. **修改前后对比**：如果是修改已有章节，必须贴出修改前/后的关键段落对比

**禁止**：仅写"文档已更新"而不附任何证据。

**核验责任**：参谋在核验执行报告时，必须对照上述证据要求逐一核查。如执行者未附证据，参谋不得通过核验，必须退回要求补充。

### 3.9 数据库迁移校验机制（自愈）

**背景**：迁移脚本的 checksum 采用 `inspect.getsource()` 获取完整源码计算，导致注释/空行变更触发误报。

**机制**（`database.py` 中实现）：

- **标准化校验**：剥离注释和 docstring 后计算 SHA-256（AST 行号定位 + 原始字符串过滤）
- **三级自愈比较**：
  1. 存储值 == 标准化值 → 直接通过
  2. 原始值 != 标准化值 → 仅注释/空行变化，强制将存储值更新为标准化值
  3. 原始值 == 标准化值，但存储值 != 标准化值 → 真实 DDL 变更，抛 `DatabaseError` 阻断启动
- **反射扫描**：自动发现 `_migrate_v*` 函数，无需手动维护版本列表
- **日志**：自动更新时输出 INFO 日志（含新旧值前 8 位）
