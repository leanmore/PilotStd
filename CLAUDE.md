...
- **标准化校验**：剥离注释后计算 SHA-256（`_norm_source()` 移除 `#` 注释和空行，保留代码逻辑行）
- **三级自愈比较**：
  1. 标准化 checksum 匹配 → 直接通过
  2. 标准化不匹配但原始 checksum 匹配 → 仅注释/空行变更，自动更新为标准格式
  3. 两者均不匹配 → 真实 DDL 变更，抛 `DatabaseError` 阻断启动
