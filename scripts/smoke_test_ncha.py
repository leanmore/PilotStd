#!/usr/bin/env python
# 模块：scripts/smoke_test_ncha.py
"""NCHA 适配器冒烟测试 — 独立可执行。

用途：验证 NCHAAdapter 在真实网络环境下正常访问 bz.ncha.gov.cn:9005。
用法：python scripts/smoke_test_ncha.py
"""
...
def main() -> int:
    ...
    for field in ["standard_number", "standard_name", "status", "publish_date"]:
        ...
        assert getattr(r, field) or field != "standard_number", f"字段 {field} 为空"
