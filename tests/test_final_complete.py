...
class TestTasks(unittest.TestCase):
    ...
    def test_get_download_url_none(self):
        ...
        db = MockDatabase("CREATE TABLE IF NOT EXISTS standard_info_cache (id INTEGER PRIMARY KEY, standard_number TEXT, result_json TEXT, cached_at TEXT)").__enter__()
        try:
...
class TestManager(unittest.TestCase):
    ...
    def test_facade_core_dataclass(self):
        ...
        core = ManagerCore(
            cfg=MagicMock(), db=MagicMock(), parser=MagicMock(), scanner=MagicMock(),
            query_engine=MagicMock(), cache=MagicMock(), quota_tracker=MagicMock(),
            adapter_manager=MagicMock(), file_index=MagicMock(), download_engine=MagicMock(),
            session_mgr=MagicMock(), task_queue=MagicMock(), router=MagicMock(),
        )
...
class TestSmallModules(unittest.TestCase):
    ...
    def test_cli_commands_all_importable(self):
        for mod_name in ("scan", "query", "download", "organize", "auto", "pending",
                          "normalize", "announce", "task", "move", "expire", "validity"):
            try:
