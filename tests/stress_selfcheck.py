...
qt._ensure_date()
_check("配额: 跨天检测", qt._today == str(date.today()), f"_today={qt._today}")
...
_check(
    "备份: 文件生成",
    os.path.exists(bak_path) and os.path.getsize(bak_path) > 0,
...
_check("文件: sanitize 无非法字符", all(c not in _result for c in '<>:"/\\|?*'))
...
_check(
    "路径: 子目录通过",
    _mover._is_safe_path(os.path.join(OUTPUT_DIR, "GB 国家标准", "subdir", "test.pdf")),
)
_check(
    "路径: 越界拒绝",
    not _mover._is_safe_path(os.path.join(OUTPUT_DIR, "..", "outside.pdf")),
)
...
fix.upsert(
    "D:/_stress_clear_1d.pdf",
    logical_code="GB",
    number=1,
    year=2020,
    std_name="近期记录",
    file_hash="hash_1d_stress3",
)
fix.upsert(
    "D:/_stress_clear_8d.pdf",
    logical_code="GB",
    number=2,
    year=2020,
    std_name="过期记录",
    file_hash="hash_8d_stress3",
)
db.execute(
    "UPDATE file_index SET last_checked=? WHERE file_hash=?",
...
_check(
    "版次跳过: 10版",
    _info is not None and _info.year == 2004,
