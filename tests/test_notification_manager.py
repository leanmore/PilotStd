...
class TestNotificationManager(unittest.TestCase):
    ...
    def test_init_disabled_skips_channels(self):
        ...
        nmgr = NotificationManager(self.mock_cfg, self.mock_db)
    ...
    def test_init_channels_only_enabled(self, _wc, _tg, _fs, _dt):
        """仅初始化已启用的渠道。"""
        self.mock_cfg.get.side_effect = lambda key, default=None: {
            ...
            "notification.channels.dingtalk.enabled": False,
        }.get(key, default)
        nmgr = NotificationManager(self.mock_cfg, self.mock_db)
