...
_VALIDITY_SCHEMA = """CREATE TABLE IF NOT EXISTS standard_validity (
    id INTEGER PRIMARY KEY, standard_number TEXT UNIQUE, status TEXT DEFAULT '未知',
    last_checked_at TEXT, next_check_at TEXT, last_status TEXT,
    last_status_updated_at TEXT, last_changed_at TEXT, check_count INTEGER DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS standard_validity (id INTEGER PRIMARY KEY, standard_number TEXT, data_state TEXT DEFAULT 'valid');
CREATE TABLE IF NOT EXISTS announcement_record (id INTEGER PRIMARY KEY, data_state TEXT DEFAULT 'valid');
...
_FILE_INDEX_SCHEMA = """CREATE TABLE IF NOT EXISTS file_index (
    id INTEGER PRIMARY KEY, filepath TEXT UNIQUE, filename TEXT, logical_code TEXT,
    standard_number TEXT, standard_name TEXT, year INTEGER, kind TEXT, language TEXT,
    file_hash TEXT, file_size INTEGER, modified_at TEXT, indexed_at TEXT);"""
...
class TestValidityChecker(unittest.TestCase):
    ...
    def test_register_duplicate_skips(self):
        ...
        vc.register_new_standard("GB/T 1.1-2020")
        vc.register_new_standard("GB/T 1.1-2020")
        rows = self.db.fetchall("SELECT COUNT(*) as cnt FROM standard_validity WHERE standard_number=?", ("GB/T 1.1-2020",))
    ...
    def test_get_due_standards(self):
        ...
        self.db.execute("INSERT INTO standard_validity (standard_number,status,next_check_at,created_at,updated_at) VALUES (?,'未知',?,?,?)", ("GB/T 1.1-2020", past, past, past))
        vc = ValidityChecker(self.db)
    ...
    def test_count_due_standards(self):
        ...
        from pilotstd.core.validity_checker import ValidityChecker
        past = (datetime.now(timezone.utc) - timedelta(days=1)).isoformat()
        self.db.execute("INSERT INTO standard_validity (standard_number,status,next_check_at,created_at,updated_at) VALUES (?,'未知',?,?,?)", ("TEST-1", past, past, past))
        vc = ValidityChecker(self.db)
