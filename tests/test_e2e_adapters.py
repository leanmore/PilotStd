...
class TestE2EStdGov(unittest.TestCase):
    ...
    def test_gb_exact_match(self):
        ...
        self.assertEqual(r.match_status, "exact",
                        f"期望 exact，实际 {r.match_status}")
        self.assertIn("质量", r.standard_name or "",
                     f"标准名称应包含'质量': {r.standard_name}")
...
class TestSiteUnavailable(unittest.TestCase):
    ...
    def test_unavailable_site_returns_none_or_error(self):
        ...
        self.assertTrue(has_error,
                       f"应返回 None 或带 error_message/not_found: {r.error_message}")
