...
| 能力类型 | 必须放在 | 禁止放在 |
|---------|---------|---------|
| 通用基础设施（DB、配置、日志、通知、缓存、文件） | Core 层 | 任何端特有层 |
| 独立业务域（query/download/scan/announcement/task） | 业务层 | Core 层或端特有层 |
| 跨域编排、统一门面 | Manager 层 | Core 层或业务层 |
| HTTP 路由、请求解析 | Web 端特有层（`docker/api/`） | 共享层（`pilotstd/`） |
| 命令行解析、输出格式化 | CLI 端特有层（`cli/`） | 共享层（`pilotstd/`） |
| Qt 组件、信号/槽、界面布局 | WinUI 端特有层（`ui/`） | 共享层（`pilotstd/`） |

### 8.2 门禁放置规则

| 门禁类型 | 执行位置 | 守护范围 |
|---------|---------|---------|
| 代码规范类 | pre-commit | 本地开发环境 |
| 构建验证类 | CI（构建阶段） | PR 和主分支 |
| 运行时检查类 | CI（测试阶段） | PR 和主分支 |
| 静态分析类 | pre-commit + CI | 本地 + PR |
...
| 违规模式 | 检查方式 | 修复方式 |
|---------|---------|---------|
| 在端侧实现通用业务逻辑 | 代码审查 | 移到共享层 |
| 共享层导入端特有代码 | `grep -rn "from docker" pilotstd/` | 移除导入，通过 Manager 调用 |
| Core 层导入上层代码 | `grep -rn "from pilotstd.manager" pilotstd/core/` | 移除导入，注入依赖 |
| 新增能力未登记 | 代码审查 | 更新能力登记簿 |
| 修改能力未更新门禁 | 代码审查 | 更新门禁索引 |
