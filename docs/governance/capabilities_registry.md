...
| 日期 | 版本 | 变更内容 |
|------|------|---------|
| 2026-06-29 | v3.0 | 新增项目治理总纲 + 门禁索引；能力登记簿增加守护门禁字段；AdapterManager 登记；A2 通知增强登记 |
| 2026-06-27 | v2.7 | 新增 GATE-09 CHANGELOG 版本一致性门禁 + GATE-10 UI 敏感字段保护门禁；CHANGELOG.md 历史版本补全（v0.5.27→v0.37.10，88个版本）|
...
| 能力 | 守护门禁 | 所在层次 |
|------|---------|---------|
| 路径白名单（path_guard.py） | GATE-01 | Core 层 |
| 日志标签（logger.py） | GATE-02 | Core 层 |
| 敏感字段掩码（settings.py） | GATE-03, GATE-14 | Core 层 |
| API 路由文档 | GATE-04 | API 层 |
| 适配器列表（_ALL_ADAPTER_NAMES） | GATE-05 | Manager 层 / 业务层 |
| Docker 挂载配置 | GATE-06 | 基础设施层 |
| Python 代码质量 | GATE-07a | 全部 Python 层 |
| TypeScript 代码质量 | GATE-07b | Web 端特有层 |
| 依赖管理 | GATE-08 | 基础设施层 |
| 版本号同步 | GATE-09 | 基础设施层 |
| UI 敏感字段 | GATE-10 | Web 端特有层 |
| 硬编码规范 | GATE-11, GATE-13 | Web 端特有层 + 共享层 |
| Vue 组件规范 | GATE-12 | Web 端特有层 |
| OCR 掩码一致性 | GATE-14 | Web 端特有层 |

---

## 能力放置约束

参考 `docs/architecture_layers.md`：

| 层次 | 允许放置的能力类型 |
|------|-------------------|
| Core 层 | 通用基础设施（DB、配置、日志、通知、缓存、文件、有效性检查） |
| 业务层 | 独立业务域（query/download/scan/announcement/task 等） |
| Manager 层 | 依赖组装、跨域编排、统一门面 |
| Web 端特有层 | HTTP 路由、请求解析、响应封装 |
| CLI 端特有层 | 命令行解析、输出格式化 |
| WinUI 端特有层 | Qt 组件、信号/槽、界面布局 |
...
### 新增能力时
1. 在本登记簿中添加一行
2. 确定能力应放在哪个层次（参考架构报告）
3. 确定守护该能力的门禁（参考门禁索引）
4. 如果是非功能性能力，需要至少一道门禁守护

### 重构/迁移时
1. 查询本登记簿，确认涉及哪些能力
2. 运行 `bash scripts/extract_capabilities.sh` 提取当前能力快照
3. 执行重构后，更新本登记簿中能力的位置信息
4. 运行门禁确认守护能力未丢失

### 移除能力时
1. 在本登记簿中标记为"已移除"
2. 确认移除该能力对应的门禁是否需要调整
3. 更新门禁索引
