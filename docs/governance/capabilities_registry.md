...
| 引擎线程池 | `pilotstd/query/engine.py` | L845 | L845 | active | 8 worker 并发执行各查询桶，桶内串行 + 桶间并行 |
| 引擎 PROGRESS 日志 | `pilotstd/query/engine.py` | L434, 1126 | L434, 1126 | active | 输出已完成/总数/成功/速率/预计剩余；完成时 eta=0s |
| 引擎 BUCKET 日志 | `pilotstd/query/engine.py` | L452, 1009 | L452, 1009 | active | 分桶分配时输出总数；完成后输出 total/done/overflow/elapsed |
| 引擎 FUNNEL 日志 | `pilotstd/query/engine.py` | L1099 | L1099 | active | 汇总输出 total/ok/overflow/pending 四维漏斗 |
| 引擎 TIMELINE 日志 | `pilotstd/query/engine.py` | L1017, 1106 | L1017, 1106 | active | 桶并发耗时 + 逐桶查询完成总耗时 |
| 引擎 BASELINE 日志 | `pilotstd/query/engine.py` | L1111 | L1111 | active | FUNNEL 同款字段 + 总耗时，供压测驱动对比基线 |
| 引擎 CACHE 日志 | `pilotstd/query/engine.py` | L1090 | L1090 | active | 批量查询结束后输出 hit/miss/rate% |
| 引擎 QUOTA/WATER 日志 | `pilotstd/query/engine.py` | L926, 936, 1024, 1061, 1067 | L926, 936, 1024, 1061, 1067 | active | 站点溢出不可用/配额耗尽/各站已用量/水位剩余/冷却跳过 |
| 引擎 SCORE 日志 | `pilotstd/query/engine.py` | L1046 | L1046 | active | 各站点 exact/fuzzy/older/mismatch 评分分布 |
| 引擎 OVERFLOW 日志 | `pilotstd/query/engine.py` | L1032 | L1032 | active | 溢出事件计数 + 链分布 |
| 引擎 CHAIN/PENDING 日志 | `pilotstd/query/engine.py` | L1051, 1055 | L1051, 1055 | active | 每条标准的查询站点链 + 待确认归因 |
...
| 压力测试双流读取 | `tests/stress_driver.py` | L322-323 | L322-323 | active | 两个 daemon 线程并行读取子进程输出，防管道缓冲区死锁 |
| 压力测试 Web 心跳 | `tests/stress_web.py` | L112, 784 | L112, 784 | active | 与 engine 层格式统一的 60s 进度日志；完成消息在 L784 |
