...
| 版本 | v1.1 |
| 批准日期 | 2026-07-14 |
| 最后更新 | 2026-09-21（新增 3.4「只评判入库产物」、3.5「检查跟随产物位置」，来源：G-012 对本地草稿产生 22 项假阳性阻断的实测） |
...
门禁的**输入必须落在可靠位置**：它只应评判要进入仓库的内容，不得评判本地草稿。

- 依据：`logs/` 等目录已由 `.gitignore` 声明为本地草稿区（一次性脚本、临时探针），它们不是仓库内容，其注释密度/格式与仓库质量无关；
- 反例（2026-09-21 实测）：G-012 注释密度 + G-010 代码规模全量扫描把 `logs/*.py` 一并判定，产生 22 项阻断违规与 2 项超长函数阻断，导致与草稿毫无关系的提交无法通过 —— 门禁的"假阳性"与"假绿"同样有害：前者逼人绕过门禁，后者让门禁失去意义；
- 落地：G-010（代码规模）、G-012（注释密度）统一经 `scripts/_gate_paths.py` 的 `is_git_ignored()` 跳过忽略路径（忽略集合由 `git ls-files --others --ignored --exclude-standard --directory` 计算，git 不可用时回退为不豁免）。

### 3.5 检查跟随产物位置

同一条判据也适用于"检查放在哪里跑"：输入依赖本机状态或待提交变更集的检查必须挂在本地（pre-commit），输入是入库产物的检查才放 CI。详见 [gates.md](gates.md) 的"放置原则"。
