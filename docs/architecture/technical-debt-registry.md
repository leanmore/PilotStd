...
> 2026-09-26 追加：同一问题（本登记簿编号 #21 = `docs/technical-debt.md` 第二节 #15）**正式关闭为已接受**——实测收益≈0（生产链路源头按批汇总、Web/定时侧本就持有长生命周期门面），而共享聚合器会让“最后入队者”决定全部缓冲消息的收件身份（缓冲键只有 `event_type`，无 `user_id` 分桶）→ 属正确性洞而非性能取舍。构造点实测：生产 **12 处** / 测试 **22 处**，不做单例化。
...
| 21 | 通知聚合器实例不共享 → 聚合对"每条通知新建门面"的路径失效 | 中 | 2026-09-21 | ✅ 已接受（2026-09-26 正式关闭） | **根因**：聚合器生命周期绑在门面上——`NotificationManager.__init__` 按 `notification.aggregate_enabled`（默认 true）在实例内新建 `NotificationAggregator` 并把 `self._send_now` 作为回调，而 `StandardManager()` 每次都新建门面 → 每构造一个门面多一个聚合器。实测（`62fba6ef`）：连建 2 个门面 `aggregator is` = **False**，3 次逐条 `send_event` 每实例缓冲恒 1 条；共享聚合器入队 3 条则缓冲 3、flush 1 次。构造点 AST 重测 = **生产 12 处 / 测试 22 处**（`tests/test_manager.py` 20 处；上一轮记 9 处已过时）。**不做单例化的理由**：聚合器回调捕获首个入队管理器的 `_send_now`（闭包 `_db`/`_ws_broadcast`/`_user_id`），共享会让最后入队者决定全部缓冲消息的收件身份，且缓冲键只有 `event_type`（无 `user_id` 分桶）→ 跨用户合并；共享整个管理器又与 `_init_notification`（配置变更须拿新实例，`tests/test_notification_api.py:86/107` 断言）冲突。**收益实测≈0**：生产链路源头按批汇总（`process_chain(notify_per_record=False)`，每次运行 1 条汇总），Web/定时侧本就持有长生命周期门面；`NotificationManager()` 构造中位 **0.62 ms**，门面构造 8.17 s 的 98.9% 在 `ssl.create_default_context`（与本条无关）。**代价（已接受）**：`favorite_chain_processor.py:307` 每条 abandoned、`favorite_download.py:306` 每次下载失败仍各自新建门面 → 通知退化为"延迟默认 5s + 每事件 1 次渠道请求"，交互型高频通知存在限流余量。详见 [technical-debt.md 第二节 #15](../technical-debt.md) |
