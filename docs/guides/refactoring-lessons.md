...
**时机**：动手拆之前，不是拆完跑门禁才发现。密度不足就返工，等于白跑一轮门禁。

**做法**：写文件前先估算新模块的 `注释行 / 非空行`，目标 **≥3%**。

- 只对 **≥50 逻辑行**（非空非注释行）的模块检查，短模块不受约束。
- 估算按"非空行"作分母（与 `scripts/check_g_012_comment_density.py` 口径一致）；早期拿不到准确值时按"非空行 ≈ 逻辑行"保守估。
- 不足就在新模块头部写清 **"拆出原因 + 分组依据"** 这类**有用**注释，而不是凑数空注释。

**依据**：门禁 **G-012**（注释密度 ≥3%），脚本 `scripts/check_g_012_comment_density.py`。

**同一坑已三次**（非偶发，都是"拆完、跑门禁"之后才暴露）：

| 时间 | 新模块 | 首版密度 | 结果 |
|------|--------|---------|------|
| TD-22（2026-09-26） | `scripts/_sql_schema_parser.py` | **1.9%** | 被 G-012 拦下 |
| 第八轮任务 1 | `pilotstd/core/notification/_builders_task_results.py` | **2.38%** | 被 G-012 拦下 |
| 第八轮任务 B | `pilotstd/query/adapters/gongbiaoku.py` | **2.99%** | 因拆分新增 2 行代码跌到 2.99%，被拦；补一条注释后 **3.70%** |

**根因**：拆出去的往往正好是"纯数据表 / 纯函数"块，说明性注释留在原文件，新模块只剩代码。

### 2. 拆分证据与行数红线（G-010）

- 新模块摆放方式必须能被 G-010 识别为拆分证据：同目录 `{文件名}/` 子目录，或 `{文件名}_*.py`。
- 500 行是**阻断档**（>400 警告、>500 阻断）；本篇记录的首轮治理（32 函数 + 11 文件包化）即是此红线驱动。
- 建议：新模块一开始就压到 400 行以内，避免拆一次不够再返工。
