...
| 28 | **可偿还** | 迁移 checksum 的“仅注释变化”自愈判定恒真 → **P-106 实际未被强制执行** | `pilotstd/core/db/_migration_checksum.py:94-107`（判定点 `:96`） | ⏳ 待偿还 | **根因**：`:96` 用「**当前**源码 `raw != norm`」判定“仅注释/空行变化”，而 `norm_source()`（`:42-59`）会 `line.strip()` **去掉行首缩进**（保留 docstring 等字符串字面量）→ 对**任何带缩进的迁移函数**该条件恒真，于是**任何** checksum 不匹配（含真实的逻辑改动）都走自愈分支：只记 WARNING + 静默 `UPDATE _schema_version.checksum`，`:109-115` 的 `DatabaseError` 在真实源码形态下**不可达**。**现状**（2026-09-26 实测）：`MIGRATIONS` 全部 **59/59** 个函数 `raw != norm`（`raw == norm` 为 0）；只有 `tests/test_core.py` 用 mock 把两者凑相等才触达抛错分支。**偿还窗口**：**最迟第十轮**。修复方向：判定改为基于“**存储值 vs 新旧 norm**”——若 `stored == raw_expected`（说明库内是历史 raw 风格哈希）才自愈到 norm，否则即为真实变更 → 报错；并补一对“改逻辑必报错 / 改注释可自愈”的测试。**不还的代价**：P-106（已执行迁移源码不可变）的保护是**名义上的**——有人改了已执行迁移的逻辑，校验不会阻断启动，只留一条 WARNING 并改写库内 checksum，生产库会在不知情的情况下带着与迁移源码不一致的 checksum 继续运行；此外每台旧库每次打开都会因自愈路径多写一次 `_schema_version`。 | 2026-09-26 |
