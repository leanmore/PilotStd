...
| 12 | G-012 LANG 历史警告（44 项） | `pilotstd/core/notification/`、`docker/api/announce_detail.py`、`pilotstd/core/config/` 等（批次 2/3/4 已提交注释含英文术语，如 appdata/DB/fallback/corpid 不在白名单） | G-012 LANG 警告（不阻断） | 不阻断提交（仅 hard error 阻断，LANG 为警告）；建议后续专项清理：将注释中的英文术语改写为中文或补充白名单（`check_g_012_comment_density.py` LANG_WHITELIST）；低优先级，Boy Scout Rule 随改随清 | 2026-08-24 |
