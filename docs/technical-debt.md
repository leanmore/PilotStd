...
> 2026-09-26 第六轮（技术债收尾：验证 → 消除 → 记录）：
> ① **#18 巡检改走公开 API 复核**——`GET /api/favorites/export?format=json` 无分页截断（导出行数 105 == 库内行数 105），A/B/C 三组的 `user_favorites` 维度可本地算得全 0；`favorite_downloads`/`announcement_record`/`download_queue` 三表列**未在任何公开端点暴露**（`favorites.py:341-346` 只给 4 个下载字段），其维度仍经 `POST /query` 取证，结果同为 0。
> ② **#11 紧急项已还**：`scripts/check_g_012_sql_schema.py` 有效行 **497 → 139**（拆出 `scripts/_sql_schema_parser.py`），`docker/auth.py` **490 → 394**（拆出 `docker/_static_token.py`）；G-010 警告区 **10 → 8**，最高 487，无文件再逼近 500 线。
> ③ **#15 现状更新**（口径扩大，仅记录不修）：`StandardManager()` 构造点实测 **9 处**（原台账只记链路 4 处），其中 2 处是**逐条通知**路径。
> ④ **#21 新登记**（下载链 `verifyCode` 恒被拒，不修只登记）；**#19/`POST /query`** 待决策项补全机制细节；`docs/deployment/README.md` 过时内容**已顺手更新**。
> ⑤ 第五节 5 条已接受设计补齐「依据/根因 + 现状」审计轨迹。
...
|---|------|------|------|------|--------------------------------------|---------|
| 11 | **ROI 判断** | G-010 警告区文件（拆分收益低于成本） | 见下方"现状"列出的 **8** 个文件 | ⏳ 待最终判断 | **根因**：历史累积复杂度进入 400-500 行警告区；集中拆分 ROI 低于组件碎片化风险。**现状**（2026-09-26 实测 `check_g_010_code_size.py`）：警告区由 10 个降至 **8 个**——本轮已拆掉两个高危项（`scripts/check_g_012_sql_schema.py` 497→139、`docker/auth.py` 490→394，见 TD-22，两者均已退出警告区），剩余：`pilotstd/core/notification/manager.py:487`、`web/src/views/AnnounceDetail.vue:487`、`pilotstd/core/notification/_builders_batch.py:477`、`scripts/check_g_012_comment_density.py:457`、`docker/api/announce_detail.py:454`、`pilotstd/core/db/_migrate_v16_v49.py:438`、`web/src/components/AppLayout.vue:434`、`web/src/components/NotificationConfig.vue:425`；**无文件 ≥490，距 500 阻断线最近的是 487（余 13 行）**，无阻断。**偿还窗口**：**最迟第七轮做最终判断**——要么挑最接近 500 行的 2-3 个拆分，要么正式转"已接受"并关闭本条；触发式提前——任一文件触及 490 行 → **当轮**必拆（本轮即按此规则处理了 497/490 两个文件）。**不还的代价**：警告区文件距阻断线越近，任何小改动都可能撞线导致 **CI 阻断**，届时被迫在红线上临时拆分。 | 2026-08-23 |
| 15 | **ROI 判断** | 通知聚合器实例不共享 → 聚合对"每次新建门面"的路径失效 | 聚合器：`core/notification/manager.py:118-135`（`_aggregate_enabled` 为真时在实例内新建）；管理器创建点：`manager/facade/_base.py:250`（门面 `__init__`）、`:271`（`_init_notification` 配置变更重建）；**`StandardManager()` 构造点实测 9 处**：`tasks/favorite_download.py:134/162/191/299`、`services/favorite_chain_processor.py:164/307`、`tasks/date_reminder.py:141`、`monitor/scheduler.py:146`、`core/task_status.py:76` | ❌ 暂缓 (Won't Fix Now) | **根因**：链路每条通知都新建门面 → 新管理器 → **新聚合器**，缓冲恒 1 条，聚合对链路完全失效。**现状**（2026-09-26 实测，口径比原登记扩大）：构造点由原记录的 4 处增至 **9 处**（跨 5 个模块）；其中 **2 处是逐条通知路径**——`favorite_chain_processor.py:307`（每条 abandoned 记录一次）与 `favorite_download.py:134`（每次下载失败一次）→ 这两条路径上聚合器**每通知一个实例**，聚合恒不生效。实际损害已被"链路源头按批汇总"消除（一次运行 1 条汇总）。**偿还窗口**：**最迟第八轮做最终判断**——要么做单例化，要么正式转"已接受设计"并关闭本条。**不还的代价**：①交互型高频通知一旦出现，仍可能触发渠道限流（此前实测 Telegram 丢失 55.6%）；②每份额外门面都多一次管理器 + 聚合器构造开销（当前量级下不明显）。 | 2026-09-21 |
