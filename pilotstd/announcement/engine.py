...
class AnnounceEngine:
    ...
    def __init__(self, adapters: list[BaseAnnounceAdapter],
                 matcher: AnnouncementMatcher):
    ...
    def check_all(self, since_date: str = "",
                  ocr_provider=None) -> dict:
        """GB→HB→DB 依次串行抓取。
        ...
        for std_type in ("gb", "hb", "db"):
            ...
            try:
                ...
                result[std_type] = {"matched": 0, "updated": 0,
                                    "total_announcements": 0,
                                    "last_notice_date": "", "error": str(e)}
        return result

    def check_one(self, standard_type: str, since_date: str = "",
                  ocr_provider=None,
                  progress_callback=None, checkpoint_pids=None) -> dict:
        """指定类型抓取。"""
        adapter = self._adapters.get(standard_type)
        if adapter is None:
            return {"error": f"未知公告类型: {standard_type}"}
        return self._check_one_adapter(adapter, since_date,
                                        ocr_provider=ocr_provider,
                                        progress_callback=progress_callback,
                                        checkpoint_pids=checkpoint_pids)

    def _check_one_adapter(self, adapter: BaseAnnounceAdapter,
                           since_date: str, ocr_provider=None,
                           progress_callback=None, checkpoint_pids=None) -> dict:
        try:
            items = adapter.fetch_announcements(
                since_date=since_date, ocr_provider=ocr_provider,
                progress_callback=progress_callback,
                checkpoint_pids=checkpoint_pids)
        except Exception as e:
            logger.error("公告适配器 %s 异常: %s", adapter.source_site, e)
            return {"matched": 0, "updated": 0, "total_announcements": 0,
                    "last_notice_date": "", "error": str(e)}
        if not items:
            return {"matched": 0, "updated": 0, "total_announcements": 0,
                    "last_notice_date": ""}
