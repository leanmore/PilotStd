...
class OrganizerService:
    ...
    def organize(self, parsed_list: list,
            ...
            if src and os.path.isfile(src):
                ...
                if self._is_word_or_template(src):
                    ...
                    if fhash and fhash in _content_hashes:
                        logger.info("内容重复，跳过: %s (已归档为 %s)",
                                    os.path.basename(src), _content_hashes[fhash])
        ...
        logger.info(f"归类完成: {result['moved']} 已移动 (Word: {result['word_mirrored']}), "
    ...
    def organize_fallback(self, source_root: str) -> dict:
        ...
        logger.info(f"兜底镜像完成: {result['moved']} 已移动, {result['skipped']} 已跳过"
