...
# StandardManager 一键处理混入模块
"""AutoMixin：一键自动运行管线（扫描→查询→下载→归档→收容）。"""
...
class AutoMixin:
    ...
    def auto_run(self, root_path: str) -> dict[str, int]:
        ...
        report: dict[str, int] = {"scan": 0, "query_found": 0, "download_success": 0, "organize_moved": 0}
    ...
    def _auto_stage_query(
        ...
        """查询阶段：包装进度回调（90→99%）→ 调用 query → 更新 report。"""
        ...
        results, q_stats = self.query(parsed, progress_callback=_progress_cb, result_callback=on_query_result)
        if on_query_progress:
    ...
    def auto_run_stream(
        ...
        report, t_stage = self._auto_stage_query(parsed, on_query_progress, on_query_result, report, t_stage, _time)
        ...
        return self._auto_stage_archive_and_fallback(
            parsed, root_path, on_archive_result, on_stage_change, report, t_stage, _time
        )
