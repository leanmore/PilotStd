...
class ScanMixin:
    ...
    def _scan_single_file(self, file_path: str):
        ...
        if parsed:
            ...
            self._mgr.upsert_file_index(
                file_path, parsed.logical_code,
                parsed.number, parsed.year, part=parsed.part,
                std_name=parsed.std_name, status="现行")
        else:
    ...
    def _scan_directory(self, dir_path: str):
        ...
        self._scan_worker = ScanWorker(self._mgr, dir_path,
    ...
    def _on_scan_batch_ready(self, batch_rows: list):
        ...
        for seq, parsed in batch_rows:
            self._parsed_results.append(parsed)
            self._add_table_row(RowUpdate(seq=seq, parsed=parsed, work_status="已扫描",
    ...
    def _on_scan_finished(self, success: int, failed: int):
        ...
        self.status_changed.emit(
            f"扫描完成: {total} 个文件, {success} 个识别成功, "
            f"{failed} 个无法识别")
