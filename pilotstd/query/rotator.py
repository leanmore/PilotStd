...
class SiteRotator:
    ...
    def _get_type_match_score(self, adapter_name: str, std_type: str) -> float:
        """计算适配器与标准类型的匹配度。"""
        if not std_type:
            return 0.0
        from ..query.search_strategy import ADAPTER_TYPE_MAP as _TYPE_MAP

        mapping = _TYPE_MAP.get(std_type.lower(), {})
        if not mapping:
            return 0.0
        if adapter_name == mapping.get("primary"):
            return 20.0
        if adapter_name == mapping.get("fallback"):
            return 10.0
        if adapter_name == "csres":
            return 5.0
        return 0.0
    ...
    def score_adapter(self, adapter_name: str, std_type: str = "") -> float:
        """计算适配器在当前状态下的适用分数（0-100）。

        评分维度：
          - 剩余配额：0-40 分
          - 冷却状态：0 或 30 分
          - 类型匹配：0-20 分（委托 _get_type_match_score）
          - 历史成功率：0-10 分
        """
        with self._lock:
            ...
            # 1. 剩余配额分（40 分）
            remaining = site.max_requests - site.request_count
            quota_score = min(remaining / 50.0, 1.0) * 40.0

            # 2. 冷却状态分（30 分）——不在冷却中即满分
            cooldown_score = 30.0

            # 3. 类型匹配分（20 分）
            type_score = self._get_type_match_score(adapter_name, std_type)

            # 4. 历史成功率（10 分）
            success_score = self._get_success_rate(adapter_name) * 10.0

            return round(quota_score + cooldown_score + type_score + success_score, 1)

    def get_available_adapters(  # type: ignore[override]
        self,
        priority: list[str] | None = None,
        std_type: str = "",
        quota_tracker=None,
    ) -> list[str]:
        """按实时评分降序返回可用适配器列表。指定 std_type 时启用类型匹配加分。"""
        candidates = priority if priority else list(self._sites.keys())
        scored = []
        for name in candidates:
            score = self.score_adapter(name, std_type)
            if score > 0:
                # 配额检查（可选）
                if (
