...
def get_settings(mgr=Depends(get_manager_dep)):
    ...
    return {
        "version": __version__,
        "storage": {"root_dir": cfg.get("storage.root_dir", ""),
        ...
        "appearance": {"theme": cfg.get("appearance.theme", "经典白"),
        ...
        "ocr": {"baidu_api_key": "***" if cfg.get("ocr.baidu_api_key") else "",
...
def put_settings(data: dict, mgr=Depends(get_manager_dep),
    ...
    """保存系统配置并更新定时任务调度。"""
    cfg = mgr.cfg
    mappings = {"storage": "storage", "organize": "organize", "file": "file",
                "network": "network", "scan": "scan", "query": "query",
                "tasks": "tasks", "appearance": "appearance", "ocr": "ocr",
    ...
    for job_id, cron_key in [("auto_scan", "auto_scan_cron"),
